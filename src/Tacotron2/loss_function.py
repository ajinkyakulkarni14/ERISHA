...
class TacotronMultilingual2Loss(nn.Module):
    def __init__(self, hp, update_step):
        super(TacotronMultilingual2Loss, self).__init__()
        self.expressive_classes = hp.emotion_classes
        self.speaker_classes = hp.speaker_classes
        self.language_classes = hp.language_classes
        self.speaker_encoder_type = hp.speaker_encoder_type
        self.expressive_encoder_type = hp.expressive_encoder_type
        self.language_encoder_type = hp.language_encoder_type
        self.update_step = update_step
        self.cat_lambda = hp.cat_lambda
        self.kl_lambda = hp.kl_lambda
        self.kl_incr = hp.kl_incr
    ...
    def forward(self, iteration, model_output, targets, s_id, e_id, l_id):
        ...
        # tacotron losses
        mel_out, mel_out_postnet, gate_out, _, s_prob, e_prob, l_prob = model_output
        gate_out = gate_out.view(-1, 1)
        mel_loss = nn.MSELoss()(mel_out, mel_target) + nn.MSELoss()(mel_out_postnet, mel_target)
        gate_loss = nn.BCEWithLogitsLoss()(gate_out, gate_target)
        ...
        #language_encoder_loss
        language_loss = self.get_encoder_loss(l_id, l_prob, self.language_classes, cat_lambda, kl_lambda, self.language_encoder_type)
                          
        return mel_loss + gate_loss + speaker_loss + expressive_loss + language_loss
